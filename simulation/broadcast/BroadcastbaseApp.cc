...
void BroadcastbaseApp::process_data_BroadcastMsg(char *pkt, hdr_ip* ih)
{
	...
	if (rcv_pkt.find(data->g_id())==rcv_pkt.end()) { 
		// if ((spx-mpx)>=0) { //received from FRONT
			// if (direction=='b')
				broadcast_procedure(data->g_id(),data->g_nhop(),data->g_slot_waiten());
...
void BroadcastbaseApp::broadcast_procedure(long int gid,int hops, long int slot_w){
	// int cw; //used for value of contention window
	
	int cw = 0; //conventional broadcast
